...
const StudentPersonalNotes: React.FC = () => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await api.get('/student/personal-notes');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingNote) {
        await api.put(`/student/personal-notes/${editingNote._id}`, formData);
      } else {
        await api.post('/student/personal-notes', formData);
      }
      fetchNotes();
      closeModal();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Delete this note permanently?')) {
      try {
        await api.delete(`/student/personal-notes/${id}`);
    ...
    <div className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div>
          <h1 className="text-4xl font-extrabold text-white tracking-tight flex items-center gap-4">
            My Notes <motion.div animate={{ rotate: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 3 }}><NotebookPen className="text-indigo-400" size={32} /></motion.div>
          </h1>
          <p className="mt-4 text-lg text-[var(--on-surface-variant)]">Your personal space to jot down study notes, ideas, and reminders.</p>
        </div>
        <motion.button 
      ...
      {loading ? (
        <div className="py-24 text-center">
          ...
          <p className="text-slate-500 italic">Loading your notes...</p>
        </div>
      ) : notes.length === 0 ? (
        <motion.div 
        ...
        >
          ...
          <p className="text-xl text-slate-400 font-medium">Your notepad is empty.</p>
          <p className="text-sm text-slate-500 mt-2">Start creating notes to organize your studies.</p>
        </motion.div>
      ) : (
        <LayoutGroup>
